...
public sealed class AlgorithmRepository
{
    ...
    public AlgorithmRepository(string? dataDirectory = null)
    {
    ...
    public async Task<List<AlgorithmEntry>> LoadAsync()
    {
        ...
        if (!File.Exists(DataFile))
        {
            ...
            NormalizeCode(seeds, decodeSeedLines: true);
            MergeBuiltIns(seeds);
        ...
        try
        {
            ...
            await using (var stream = File.OpenRead(DataFile))
                entries = await JsonSerializer.DeserializeAsync<List<AlgorithmEntry>>(stream, _options)
                    ?? throw new JsonException("Expected an algorithm collection.");
            NormalizeCode(entries);
            // Saved data is authoritative, including an intentionally empty library.
            return entries;
        }
        catch (JsonException)
        {
            var backup = Path.Combine(DataDirectory, $"algorithms-corrupt-{DateTime.Now:yyyyMMdd-HHmmss}-{Guid.NewGuid():N}.json");
            File.Copy(DataFile, backup, false);
            var recovered = CreateSeeds();
            NormalizeCode(recovered, decodeSeedLines: true);
    ...
    public Task SaveAsync(IEnumerable<AlgorithmEntry> entries) =>
        AtomicJsonFile.WriteAsync(DataFile, JsonSerializer.SerializeToUtf8Bytes(entries, _options));
    ...
    private static void NormalizeCode(IEnumerable<AlgorithmEntry> entries, bool decodeSeedLines = false)
    {
        foreach (var entry in entries)
        {
            if (entry is null) throw new JsonException("Algorithm records cannot be null.");
            ...
            if (decodeSeedLines && !entry.Code.Contains('\n') && entry.Code.Contains("\\n"))
