...
Windows may block unsigned native builds. The browser edition is the supported alternative in this workspace. The pattern/question bank and C# Learning Hub are browser-only; the separate native C# app is unchanged.
...
```powershell
node --test Tests/web-regression.cjs Tests/theme-contrast.cjs Tests/local-files.cjs
