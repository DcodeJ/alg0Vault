(()=>{
  ...
  function hasRoom(records) {
    if (records.length < 10000) return true;
    toast('This collection has reached the 10,000-record limit. Export a backup before removing records to make room.');
    return false;
  }
...
$('#saveNote').onclick=()=>{$$('[data-note-bind]').forEach(el=>n[el.dataset.noteBind]=el.isContentEditable?el.textContent:el.value);n.updatedAt=now();if(saveNotes()){renderNotes();toast('Thread saved')}};$('#pinNote').onclick=()=>{n.pinned=!n.pinned;n.updatedAt=now();saveNotes();renderNotes()};$('#deleteNote').onclick=()=>{if(confirm(`Delete “${n.title}”?`)){replyDrafts.delete(n.id);notes=notes.filter(x=>x.id!==n.id);selectedNoteId=notes[0]?.id;saveNotes();renderNotes()}};$('#postReply').onclick=()=>{const text=$('#replyText').value.trim();if(!text)return;if(!hasRoom(n.replies||[]))return;(n.replies??=[]).push({author:'You',content:text,createdAt:now()});n.updatedAt=now();replyDrafts.delete(n.id);const saved=saveNotes();renderNotes();if(saved)toast('Reply added')};$('#attachmentHelp').onclick=()=>alert('For security, browsers cannot silently copy local files. Add a file path, cloud link, or description in the main note. The native C# edition supports managed local attachments, but may be blocked by Windows on unsigned builds.')}
  function renderPractice(){
    stats();
    // Re-resolve against the current library and filters: a deleted/edited object may still be cached.
    const current = practicePool().find(x => x.id === practiceCurrent?.id);
    if (!current) generateChallenge();
    else { practiceCurrent = current; challenge(practiceRevealed); }
  }
  ...
  $('#backupFile').onchange = async event => {
    ...
    try {
      ...
      try {
        ...
        for (const [key,value] of [[KEY,oldAlgorithms],[NOTES_KEY,oldNotes],[LEARNING_KEY,oldLearning]]) {
          try { value===null?localStorage.removeItem(key):localStorage.setItem(key,value); }
          catch { recoveryFailed = true; }
        }
        if (recoveryFailed) {
          [KEY,NOTES_KEY,LEARNING_KEY].forEach(key => dirtyStores.add(key));
          setSaveStatus('Storage recovery failed — export your session','error');
          throw new Error('Import failed and storage recovery could not complete. Export your current session before closing.');
        }
        throw error;
      }
  ...
  window.addEventListener('beforeunload',event=>{
    flushSaves();
    if(dirtyStores.size || fileTools?.hasUnsavedFile() || [...replyDrafts.values()].some(text => text.trim())){event.preventDefault();event.returnValue='';}
  });
  ...
  $('#languageSelect').onchange = event => {
    ...
    if (currentView === 'learning') learningPage.refreshLanguage();
    fileTools?.refresh();
  };
  const learningPage = VaultLearning.create({
    highlight, copyText, flushSaves, getLanguage: () => currentLanguage,
    getProgress: () => learningProgress,
    updateProgress(id, patch, immediate) {
      let record = learningProgress.find(x => x.id === id);
      if (!record) {
        if (!hasRoom(learningProgress)) return;
        record = { id, completed: false, notes: '', updatedAt: now() }; learningProgress.push(record);
      }
  ...
  $('#addAlgorithmPack').onclick = () => {
    try { algorithms = VaultData.merge(algorithms,VaultData.algorithms(VaultCatalog.extra)); }
  ...
  const studyDesk=VaultDesk.create({
    ...
    open(kind,id){
      ...
      if(kind==='practice'){practiceRevealed=false;practiceCurrent=algorithms.find(x=>x.id===id);['practiceDifficulty','practiceCategory','practiceMastery'].forEach((key,i)=>$('#'+key).value=['Any difficulty','Any category','Any mastery'][i]);$('#practicePriority').value='';showView('practice');return}
