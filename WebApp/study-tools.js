...
(function(root) {
  ...
  function create(options) {
    ...
    const recallCard = (item, count) => recall(options.getLesson ? options.getLesson(item) : item, count);
    function refreshLanguage() {
      if (!activeRecall) return;
      const card = recallCard(activeRecall.item, activeRecall.count);
      $('#studyQuestion').textContent = card.question;
      if (activeRecall.revealed) $('#studyAnswer').innerHTML = options.markdown(card.answer);
    }
    ...
    return { mount, dashboard, queue, refreshLanguage };
  }
