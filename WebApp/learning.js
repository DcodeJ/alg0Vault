...
(function (root) {
  ...
  function lessonForLanguage(item, language = 'csharp') {
    const translated = item.translations?.[language];
    const fallback = language !== 'csharp' && !translated && /^\s*```(?:csharp|cs)\s*$/m.test(item.markdown);
    return { ...item, markdown: translated || item.markdown, languageFallback: fallback };
  }
  ...
  function markdown(source, highlight = escape) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (/^\s*```/.test(line)) {
        ...
        const normalized = ({ cs: 'csharp', 'c++': 'cpp', py: 'python' })[language] || language;
        const label = ({ csharp: 'C#', cpp: 'C++', python: 'Python', text: 'Output / text', '': 'Text' })[normalized] || language;
        result.push('<div class="learning-code-label">' + escape(label) + '</div><pre><code>' + (['csharp', 'cpp', 'python'].includes(normalized) ? highlight(code.join('\n'), normalized) : escape(code.join('\n'))) + '</code></pre>');
  ...
  function create(options) {
    ...
    const getLesson = item => lessonForLanguage(item, options.getLanguage?.() || 'csharp');
    const languageNotice = item => getLesson(item).languageFallback
      ? '<p class="learning-callout" data-i18n="lessonLanguageFallback">This lesson still uses C# / .NET examples. A version in your selected programming language is not available yet; code labels show the actual language.</p>' : '';
    ...
    function filtered() {
      ...
      return items.filter(item => (!kind || item.kind === kind)
        ...
        && words.every(word => searchText(`${item.title} ${item.english || ''} ${item.group} ${item.markdown} ${Object.values(item.translations || {}).join(' ')} ${state(item.id).notes}`).includes(word)));
    }
    function renderList() {
    ...
    function refreshLanguage() {
      const item = byId.get(selectedId);
      if (!item) return;
      $('#learningContent').innerHTML = markdown(getLesson(item).markdown, options.highlight);
      $('#learningLanguageNotice').innerHTML = languageNotice(item);
      studyTools.refreshLanguage();
    }
    function search() {
      options.flushSaves(); const visible = renderList();
      if (!visible.some(x => x.id === selectedId)) {
    ...
    const studyTools = VaultStudy.create({ ...options, getLesson, escape, markdown: text => markdown(text, options.highlight), open, render });
    return { render, open, refreshLanguage };
  }
  root.VaultLearning = { create, markdown, inline, searchText, lessonForLanguage };
