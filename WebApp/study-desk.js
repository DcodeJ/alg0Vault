(function(root){
  ...
  function search(query,algorithms,items,notes){
    ...
    const entries=[...algorithms.map(x=>({kind:'algorithm',id:x.id,title:x.title,meta:x.category+' · '+x.priority,text:[x.title,x.category,x.summary,x.tags,x.notes].join(' ')})),...items.map(x=>({kind:'lesson',id:x.id,title:x.title,meta:x.kind+' · '+x.group,text:[x.title,x.group,x.markdown].join(' ')})),...notes.map(x=>({kind:'note',id:x.id,title:x.title,meta:x.course,text:[x.title,x.body,x.course,...(x.replies||[]).map(r=>r.content)].join(' ')}))];
