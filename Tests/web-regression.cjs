...
function boot(initial = {}, failWrites = false, confirmResponse = true) {
  ...
  const context = vm.createContext({ document, console, crypto: webcrypto, navigator: { clipboard: { writeText: async text => clipboard.push(text) } }, confirm: message => { confirmations.push(message); return confirmResponse; }, alert() {}, Blob,
    ...
    setTimeout: (fn) => { timers.set(++serial, fn); return serial; }, clearTimeout: id => timers.delete(id), addEventListener() {}
  });
  ...
  return { nodes, values, all, downloads, clipboard, confirmations, locale: context.VaultLocale, body: document.body, flush: () => { const callbacks = [...timers.values()]; timers.clear(); callbacks.forEach(fn => fn()); } };
}
